...
class WellfoundSource(JobSourceBase):
    ...
    DEFAULT_SEARCH_URL = "https://wellfound.com/role/ml-engineer/germany"

    def run(self):
        search_url = self.config.get("jobs", {}).get("sources", {}).get("wellfound", {}).get("search_url", self.DEFAULT_SEARCH_URL)
        try:
            ...
            log.warning("Wellfound fetch failed: %s", exc)
            return []
